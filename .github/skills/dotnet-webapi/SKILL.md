...
description: >
  ...
  USE FOR: adding new API endpoints (minimal APIs only; controllers are
  prohibited), wiring up OpenAPI/Swagger, creating .http test files, setting up
...
> **Regla obligatoria del proyecto:** los controllers están PROHIBIDOS. Todo
> endpoint DEBE implementarse con ASP.NET Core Minimal APIs siguiendo el patrón
> `ISlice` de Vertical Slice Architecture. No uses `ControllerBase`,
> `[ApiController]`, `AddControllers()`, `MapControllers()` ni `CreatedAtAction`.
> Consulta `.github/skills/minimal-api-slices/SKILL.md` y
...
- adding or modifying Web API endpoints implemented with minimal APIs;
...
- any existing minimal API registrations such as `app.MapGet`, `app.MapPost`,
  `app.MapPut`, or `app.MapDelete`, including the `ISlice` implementations;
...
so the implementation follows the established `ISlice` conventions.
## Workflow

### Step 1: Confirm the API style

The API style is fixed: **minimal APIs only**. Controllers are prohibited.

1. Scan `Program.cs` and the feature folders for existing `ISlice`
   implementations and `app.MapGet`, `app.MapPost`, etc.
2. Implement every new endpoint as a minimal API following the `ISlice` pattern
   (see `.github/skills/minimal-api-slices/SKILL.md`).
3. Never introduce `ControllerBase`, `[ApiController]`, `AddControllers()` or
   `MapControllers()`. If any controller exists, treat it as tech debt to be
   migrated to minimal APIs, not extended.
...
**Minimal API validation — register explicitly:** minimal APIs require explicit
opt-in for data-annotation validation (`[Required]`, `[MaxLength]`, `[Range]`,
etc.). For **.NET 10+** projects using minimal APIs, add the validation services
in `Program.cs`:
...
Implement every endpoint as a minimal API following these HTTP conventions
consistently.
...
Do not inject data stores directly into endpoint handlers.
...
| Using any controller construct (`ControllerBase`, `[ApiController]`, `AddControllers()`, `MapControllers()`, `CreatedAtAction`) | Prohibited. Implement endpoints as minimal APIs with the `ISlice` pattern. |
